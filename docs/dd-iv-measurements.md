...
- selected-leg and matched-delta measurements use different deterministic
  cohort IDs; and
...
Version `1.0.0` implements `MATCHED_DELTA`. A future
`MATCHED_FORWARD_MONEYNESS` basis requires a new declared profile contract;
it must not be inferred from this version.
...
- each matched-delta rule's front/back expiration, side, target, convention,
  tolerance, missing policy, temporal-skew limit, and optional interpolation
  rule.
...
front/back expirations. Delta is compared under the declared convention:

- `SIGNED_FORWARD_DELTA_PERCENT`; or
- `ABSOLUTE_FORWARD_DELTA_PERCENT`.

Direct selection is deterministic: minimum absolute delta error, then source
symbol. IV is never a tie-breaker. A contract is accepted only when its error
is within the declared tolerance. Otherwise the result is
`NOT_AVAILABLE`, with the nearest achieved coordinate, delta error, coverage
gap beyond tolerance, source strike, and source symbol retained for coverage
analysis.

The output preserves front and back achieved coordinates, delta errors,
source strikes, source symbols, IV origin, models, forward assumptions,
...
- have IV, delta, and lineage available no later than the checkpoint;
